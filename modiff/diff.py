...
class ModelDiff:
    ...
    def max_attention_diff(self, pos) -> Float[Tensor, "tasks n_heads"]:
        ...
        layers_heads = self._layers_heads()
    ...
    def plot_correct_token_log_prob_diff(self) -> Figure:
        ...
        fig = px.line(data.T)                
        fig.update_layout(
            title="Log probs of the correct token - difference between models",
    ...
    def plot_max_attention_diff(self, pos) -> Figure:
        data = self.max_attention_diff(pos).tolist()
        
        labels = [f"{layer}-{head}" for layer, head in self._layers_heads()]
        
        fig = px.line(x=labels, y=data)
        fig.update_layout(
            title=f"Max attention on postion {pos} - difference between models",
            legend_title="Average max attenton diff",
            xaxis_title="Head ix",
            yaxis_title=f"model_0 - model_1 diff of (attn[{pos}].max(-1).mean())",
            title_x=0.5,
        )                    
        return fig
    ...
    def _layers_heads(self) -> List[Tuple[int, int]]:
        ...
        for layer in range(n_layers):
            out += [(layer, head) for head in range(n_heads)]
        return out